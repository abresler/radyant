      ...
      conditionalPanel(condition = "input.datasets != ''",
        conditionalPanel(condition = "input.tool == 'data'", 
          tabsetPanel(id = "datatabs",
            # tabPanel("Manage", HTML('<label>Only (max) 20 rows shown. See View-tab for details.</label>'), tableOutput("dataexample")),
            tabPanel("Manage", HTML('<h4>Only (max) 20 rows shown. See View-tab for details.</h4>'), tableOutput("dataexample")),
